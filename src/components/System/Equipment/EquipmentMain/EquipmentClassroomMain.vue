...
import EquipmentMain from './EquipmentMain.vue';
import EquipmentClassroomSearch from '../EquipmentSearch/EquipmentClassroomSearch.vue';
...
<template>
  <div class="notice-main-container">
        ...
        <template v-if="classroomCount > 0">
          <tr v-for="classroom in classroomList" :key="classroom.roomId" class="notice-table-row">
            ...
            <td
              ...
              @click="classroomEqpDetail(classroom.roomId)"
            >
              {{ classroom.roomName }}
            </td>
    ...
    />
    <EquipmentClassroomSearch v-if="selectedClassroom !== 0" />
    <EquipmentMain v-if="selectedClassroom !== 0" :detail-id="selectedClassroom" />
  </div>
</template>
